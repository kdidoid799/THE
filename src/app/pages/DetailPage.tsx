...
export default function DetailPage() {
  ...
  const handleSaveTitle = async () => {
    await storage.updateItem(item.id, { title: editedTitle });
    setItem({ ...item, title: editedTitle });
    setIsEditingTitle(false);
  };

  // 不再需要这个条件判断，让PageTransition组件的isLoading属性处理加载状态
                  ...
                  {isEditingTitle ? (
                    <div className="flex-1 flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 px-2 py-1 border rounded-lg text-xl font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500"
                        autoFocus
                      />
